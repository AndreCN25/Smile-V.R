...
import { Eye, EyeOff, Lock, User, AlertCircle, CheckCircle, UserPlus } from "lucide-react";
import { getSettings, loginUser, registerUser } from "../../services/api";
...
export function Login({ onLogin }: LoginProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setTimeout(async () => {
      const currentEmail = localStorage.getItem("adminEmail") || DB_CONFIG.adminEmail;
      const currentPassword = localStorage.getItem("adminPassword") || DB_CONFIG.adminPassword;
      ...
      const handleLoginSuccess = () => {
        ...
        if (prevEmail && prevEmail !== email) {
          ...
          localStorage.setItem("adminPhone", "");
          localStorage.setItem("adminBio", "");
        }
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      setRegLoading(false);
      if (res.ok) {
        setRegSuccess("¡Cuenta creada exitosamente! Ahora inicia sesión.");
        setEmail(regEmail);
        setPassword("");
        setRegEmail("");
        setRegPassword("");
        ...
        {!isRegister && (
          <div className="rounded-3xl p-7 shadow-2xl" style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(20px)" }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {regSuccess && (
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm" style={{ background: "#ECFDF5", color: "#059669" }}>
            ...
            <div className="mt-5 pt-5 border-t text-center" style={{ borderColor: "var(--border)" }}>
              <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                ¿No tienes cuenta?{" "}
                <button type="button" onClick={() => { setIsRegister(true); setError(""); setRegSuccess(""); }} className="font-semibold hover:underline" style={{ color: "var(--primary)" }}>
                  Regístrate
                </button>
              </p>
            </div>
        ...
        {isRegister && (
          <div className="rounded-3xl p-7 shadow-2xl" style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(20px)" }}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="font-semibold" style={{ color: "var(--foreground)", fontSize: "1.1rem" }}>Crear cuenta</h2>
            </div>
            <p className="text-sm mb-6" style={{ color: "var(--muted-foreground)" }}>Registra una nueva cuenta para la clínica</p>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={blurHandler}
                  />
                  <button type="button" onClick={() => setShowRegPass(!showRegPass)} className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5">
                    {showRegPass ? <EyeOff className="w-4 h-4" style={{ color: "var(--muted-foreground)" }} /> : <Eye className="w-4 h-4" style={{ color: "var(--muted-foreground)" }} />}
                  </button>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--foreground)" }}>Confirmar contraseña</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "var(--muted-foreground)" }} />
                  <input
                    type={showRegPass ? "text" : "password"}
            ...
            <div className="mt-5 pt-5 border-t text-center" style={{ borderColor: "var(--border)" }}>
              <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                ¿Ya tienes cuenta?{" "}
                <button type="button" onClick={() => { setIsRegister(false); setRegError(""); }} className="font-semibold hover:underline" style={{ color: "var(--primary)" }}>
                  Inicia sesión
                </button>
              </p>
            </div>
