...
export function Appointments() {
      ...
      if (status === "confirmada" || status === "cancelada") {
        ...
        if (appt) {
          ...
          if (patient && patient.phone) {
            ...
            setTimeout(() => setWaToast({show:false,ok:false,msg:""}), 3000);
          }
        }
      }
    } catch (e) { console.error(e); }
  ...
  return (
    <div className="flex h-full overflow-hidden" style={{ fontFamily:"'DM Sans', sans-serif" }}>
      ...
      {/* WhatsApp toast */}
      {waToast.show && (
        <div className="fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-3.5 rounded-full shadow-2xl text-sm font-bold animate-in slide-in-from-bottom-8 fade-in duration-300" style={{ background: waToast.ok ? "#059669" : "#DC2626", color: "#fff" }}>
